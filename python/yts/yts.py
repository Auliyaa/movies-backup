...
def yts_get_link(req: str):
    ...
    for link in torrent_links:
        ...
        href = link.get("href")
        if not title or not href or not href.startswith("https://yts.mx"):
            continue
...
def yts_download(title: str):
    try:
        quality,uri = yts_get_link(title)
        print(f"{title} -> [{quality}] {uri}")
        if not uri:
